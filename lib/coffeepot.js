...
var CoffeePot = {
  ...
  set: function(ViewModel, Model) {
    ...
    /**
     * Number to comma 3 number.
     * @param  n  number object.
     * @return  comma 3 number. ex:'123,456'
     */
    ...
    /**
     * Number to comma 4 number.
     * @param  n  number object.
     * @return  comma 4 number. ex:'1234,5678'
     */
    ...
    ViewModel.bind = function() {
        ...
        if (typs.length >= 3) {
          ...
          for (var i = 0; i < reducerExps.length; i++) {
            ...
            if (reducerTokens) {
              ...
              var reducerArgs = reducerTokens[3] !== undefined ? reducerTokens[3].split("\|") : [];
              switch (reducerName) {
                case 'trim':
                ...
                case 'max':
                  if (reducerArgs.length > 0) {
                    var length = parseInt(reducerArgs[0]);
                    reducers.push(function(value) {return value.length <= length ? value : value.substring(0, length);});
                  }
                  break;
                case 'default':
                  if (reducerArgs.length > 0) {
                    reducers.push(function(value) {return value !== '' ? value : reducerArgs[0];});
                  }
                  break;
                case 'replace':
                  if (reducerArgs.length >= 3) {
                    var regexp = new RegExp(reducerArgs[0], reducerArgs[1]);
                    reducers.push(function(value) {return value.replace(regexp, reducerArgs[2]);});
                  }
                  break;
              }
            }
      ...
      for (var id in events) {
        ...
        for (var typ in eventSet) {
          ...
          bindEvents[typ] = createEvent(action);
        }
        obj.on(bindEvents);
      }
    };
